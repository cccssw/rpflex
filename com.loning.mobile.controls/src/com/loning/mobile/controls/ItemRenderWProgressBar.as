...
package com.loning.mobile.controls
{
	 ...
	 *  @productversion Flex 4.5
	 */
	public class ItemRenderWProgressBar extends LabelItemRenderer 
		implements IGraphicElementContainer, ISharedDisplayObject
	{
		...
		private var rectShape:Shape=new Shape();
		private var iconDisplayX:Number;
		private var hasIcon:Boolean=false;
		private var itemHeight:Number=0;
		private var labelComponentsY:Number = 0; 
		private var messageComponentsY:Number=0;
		private var progressBar:ProgressBar;
		private var loadComplete:Boolean=false;
//		var progressBar:views.components.ProgressBar;
		//--------------------------------------------------------------------------
		//
		//  Constructor
		//
		 ...
		 *  @productversion Flex 4.5
		 */
		public function ItemRenderWProgressBar()
		{
			super();
		...
		private function loadExternalImage(source:Object, iconDelay:Number):void
		{
				...
				//				progressBar.height = iconDisplay.height;
			...
			if(!progressBar && iconDisplay.source==data[iconField]){
				trace(data.title + " ++++++++++++++++++++++++ ADD LOADING");			
												   ...
												   unscaledHeight:Number):void
		{
			...
			// icon is in the middle and on the top
			...
			if (iconDisplay)
			{
				...
				//ADDED by MARK
				iconDisplayX=0;
				setElementPosition(iconDisplay, iconDisplayX, 0);
				
			}
			var viewWidth:Number;
			var viewHeight:Number;
			if(iconHeight>0){
				viewWidth  = unscaledWidth-paddingLeft-paddingRight;
				viewHeight = unscaledHeight;
			}else{
				viewWidth  = unscaledWidth-paddingLeft-paddingRight;
				viewHeight = unscaledHeight;
			}
			...
			if (decoratorDisplay)
			{
				...
				decoratorY = Math.round(0.5 * (unscaledHeight - decoratorHeight)) ;
				setElementPosition(decoratorDisplay, unscaledWidth -  decoratorWidth, decoratorY);
			...
			var labelComponentsViewWidth:Number;
			if(iconDisplay && iconHeight>0){
				labelComponentsViewWidth = iconWidth  ;
			}else{
				labelComponentsViewWidth = unscaledWidth  - decoratorWidth -paddingLeft- paddingRight;
			}
			
			var messageComponentsViewWidth:Number=labelComponentsViewWidth;
			...
			if (hasMessage)
			{
				...
				messageWidth = Math.max(messageComponentsViewWidth, 0);
				...
				else
				{
					...
					setElementSize(messageDisplay, messageWidth-5, oldPreferredMessageHeight);
			...
			if (hasLabel)
			{
				...
				if (labelWidth == 0)
					...
					setElementSize(labelDisplay, labelWidth -5, labelHeight);
			...
			totalHeight = labelAlignmentHeight + messageAlignmentHeight ;	
			
			
			
			if (labelDisplay){
				if(iconHeight>0){
					labelComponentsY =  unscaledHeight - totalHeight - 2*verticalGap  ;
				}
				else{
					if(hasMessage)
						labelComponentsY =paddingTop; //paddingTop
					else
						labelComponentsY=decoratorY;
				}
				if(iconHeight>0)
					setElementPosition(labelDisplay, 5, labelComponentsY);
				else
					setElementPosition(labelDisplay, paddingLeft, labelComponentsY);
			}
			...
			if (messageDisplay)
			{
				if(iconHeight>0){
					if(hasLabel)
						messageComponentsY = labelComponentsY  + verticalGap + labelAlignmentHeight;
					else
						messageComponentsY=unscaledHeight - totalHeight - verticalGap  ;
				}
				
				
				if(iconHeight>0)
					setElementPosition(messageDisplay, 5, messageComponentsY);
				else{
					setElementPosition(messageDisplay,paddingLeft,messageComponentsY);
				}
				
			}
			
			if(iconHeight>0){
				rectShape.width=iconWidth;
				if(hasLabel)
					rectShape.height=totalHeight + 3*verticalGap ; //totalHeight+2*verticalGap
				else
					rectShape.height=totalHeight + 2* verticalGap  ; //totalHeight+2*verticalGap
				
				rectShape.x=iconDisplayX;
				rectShape.y=unscaledHeight-rectShape.height;
			}
			if(messageDisplay && labelDisplay)
				this.setChildIndex(labelDisplay,getChildIndex(messageDisplay));
			if(labelDisplay && rectShape)
				this.setChildIndex(rectShape,getChildIndex(labelDisplay));
			
			
			
			//			progressBar.width=rectShape.width;
			rectShape.visible=true;
		}
		
		private function onIconDisplayReady(event:FlexEvent):void{
			if(iconDisplay.source==data[iconField]){
				if(progressBar && progressBar.parent){
					trace(data.title + " ONICONDISPLAYREADY+++++++++++++++++++++++++++++++++++++++++++++++");
